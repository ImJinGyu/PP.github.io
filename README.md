...
포트폴리오
        ...
        ● 개요 
        - 누구나 편리하게 숙박예약을 할수있는 웹사이트

        ● 사용언어 : JAVA1.8, JSP2.3 JavaScript, Jquery3.3.1, Ajax, JSTL
	- 사용 api : 네이버로그인, 카카오로그인, 이메일인증, 다음 주소찾기, KG이니시스 결제

	● 개발환경 
	OS/DB/Tools/ETC/WAS : Windows10 / MYSQL / Eclipse / HTML5&CSS3 / BootStrap4 / Tomcat8.5

        ● 구현목록
	 회원가입, 로그인, 패스워드 찾기, 회원정보수정 및 탈퇴, 숙박업체 검색, 페이징처리, 무한 스크롤 기능, 예약기능, 결제, 다중 파일업로드
	 댓글(리뷰), 답변, 1:1문의 및 답변, 숙박 신규등록 및 수정, 사업자 및 관리자 통계(전체매출, 회원목록, 업체목록 ...)


        #로그인 : 
                -회원,사업자,관리자 타입별로 구분하여 DB에 접속시 아이디 존재여부 확인
		 아이디 존재시 패스워드를 일치여부 확인시 불일치시 알림창 생성, 일치시 메인화면으로 이동
		 네이버,카카오 로그인시 계정 액세스 권한 요청 동의 후 토큰 발급 후 로그인 성공페이지(콜백 URL) - 가져오는 값(이메일, 이름)
		 로그인 성공시 세션값 생성(아이디, 로그인 타입, 이름)
        
        #회원가입 :
                -회원가입시에는 각 입력란에 정규식과 유효성검사를 실시하여 DB에 올바른 값들만 전송되도록 설정. 
                 
                 아이디 중복체크 확인시 입력값이 DB에 존재하는지 체크하여 유효성검사를 실시.
		 
                 이메일 인증시 랜덤값으로 6자리 숫자를 생성한 뒤 구글 SMTP프로토콜을 이용하여 입력한 이메일로 해당 인증번호
                 를 전송. 이 후 전송된 값과 입력값이 같으면 이메일 인증이 완료.
		 

        
        #마이페이지 :
		
		 1. 개인정보 수정 : 로그인된 회원에 정보를 수정 및 탈퇴
		 
		 2. 본인이 작성한 리뷰 보기 : 자신이 이용한 숙박업체를 대상으로 작성된 리뷰 통계
		 
		 3. 예약 내역조회  : 자신이 예약한 내역에 관한 세부 정보 확인(예약일자와 현재일자를 비교하여 취소가능, 예약일자가 지날시 취소불가)
		 
		 4. 1:1문의 : 관리자에게 1:1문의 내역확인 및 작성 기능
